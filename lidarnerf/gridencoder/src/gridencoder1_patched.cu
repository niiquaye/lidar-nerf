...
#define AT_DISPATCH_FLOAT_AND_HALF(TYPE, NAME, ...)                                   \
  [&] {                                                                               \
    const at::ScalarType _stype = TYPE;                                               \
    switch (_stype) {                                                                 \
      case at::ScalarType::Float: { using scalar_t = float; return __VA_ARGS__(); }   \
      case at::ScalarType::Half:  { using scalar_t = at::Half; return __VA_ARGS__(); }\
      default: AT_ERROR(#NAME, " not implemented for '", toString(_stype), "'");      \
    }                                                                                 \
  }()
    ...
    AT_DISPATCH_FLOAT_AND_HALF(
        embeddings.scalar_type(), "grid_encode_forward", ([&] {
            static_assert(!std::is_same<scalar_t, double>::value,
                          "Double precision is not supported in gridencoder kernels.");
